...
def load_data(
    ...
    # Right padding
    x_docs = torch.tensor(
        [doc + (max_length - len(doc)) * [pad_token_id] for doc in docs]
    )
    ...
    train_mask = torch.cat(
        [torch.zeros(vocab_size, dtype=torch.bool), train_mask], dim=0
    )
    ...
    assert word_mask.size(0) == train_mask.size(0) == test_mask.size(0) == num_nodes
...
def shuffle_augment(docs: list, labels: list, factor: float = 1.0, random_seed=None):
    assert factor > 0.0
