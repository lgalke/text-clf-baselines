...
from ein import init_classifier_from_embedded_class_names


PAD_TOKEN_ID = 0  # 0 is good default, exceptions handled later based on tokenizer
...
ALL_MODELS = (
    BERT_PRETRAINED_MODEL_ARCHIVE_LIST + DISTILBERT_PRETRAINED_MODEL_ARCHIVE_LIST
)
MODEL_CLASSES = {
...
def collate_for_transformer(examples):
    global PAD_TOKEN_ID
    docs, labels = list(zip(*examples))
    input_ids = torch.tensor(pad(docs, with_token=PAD_TOKEN_ID))
    attention_mask = torch.ones_like(input_ids, dtype=torch.long)
...
def run_xy_model(args):
    ...
    if args.shuffle_augment:
        ...
        new_docs, new_labels = shuffle_augment(
            list(train_docs), list(train_labels), factor=factor, random_seed=args.seed
        )
    ...
    if args.model_type != "mlp":
        ...
        if args.ein:
            print("EIN Init")
            class_names_in_order = list(
                tuple(zip(*sorted(label2index.items(), key=lambda kv: kv[1])))[0]
            )
            init_classifier_from_embedded_class_names(
                model, tokenizer, class_names_in_order
            )
    else:
...
def main():
    ...
    parser.add_argument(
        ...
        default=True,
        action="store_false",
        help="Allow updating pretrained embeddings",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help="Factor for shuffle data augmentation",
    )

    # TRF params
    parser.add_argument("--ein", default=False, action="store_true")
    ##########################
    ...
    if WANDB:
        wandb.init(project="text-clf-new")
